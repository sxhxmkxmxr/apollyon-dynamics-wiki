...
<aside class="aside">
  ...
  <ul>
    ...
    <li><a href="#loop">Sim-to-real flight loop</a></li>
...
<section id="backbone">
  <div class="sec-tag">03 · Physics backbone</div>
  <h2>The flight-grounded simulation backbone</h2>
  <p class="lede">A control policy is only ever as dependable as the fidelity of the physical environment in which it was formed. Standard simulators are written for computer graphics or for nominal, peaceful flight. A learned policy needs one specific vehicle behaving the way it actually behaves at its mechanical limits—and that simulator has to be earned, one instrumented flight at a time.</p>

  <p>The core methodology enforces relentless empirical calibration: every flight sortie is instrumented densely with high-rate black-box logging. Following recovery, the exact real-world command sequences are replayed through the simulator. Any divergence between the physical flight telemetry and simulated prediction is traced to an exact term in the physics core. <strong>None of it is averaged away as process noise.</strong></p>
...
<section id="loop">
  <div class="sec-tag">04 · Sim-to-real flight loop</div>
  <h2>What one instrumented sortie returns</h2>
  <p class="lede">Flights are flown at the structural boundary, logged densely, and analyzed within hours. What comes out of a campaign is not a slide deck. It is four concrete system upgrades, and then the next airframe.</p>

  <div class="tw">
    <table>
      <caption>One sortie, four systems upgraded</caption>
      <thead><tr><th>Flight observation</th><th>System changed</th><th>Engineering change made</th></tr></thead>
      <tbody>
        <tr><td>An unexpected actuator slew rate limit under dynamic load</td><td><strong>Simulator physics</strong></td><td>Aerodynamic torque and actuator back-EMF models updated against observed telemetry.</td></tr>
        <tr><td>Rotational acceleration mismatch during high-G pitch-up</td><td><strong>Control policy</strong></td><td>Neural policy retrained on newly mapped boundary dynamics.</td></tr>
        <tr><td>A latency jitter spike on the onboard sensor bus</td><td><strong>Onboard runtime</strong></td><td>Kernel scheduling and DMA pipeline timing audited against cycle traces.</td></tr>
        <tr><td>Battery internal resistance rising faster than predicted</td><td><strong>Hardware specification</strong></td><td>Avionics power bus, cell thermal sinking, and wiring harnesses revised.</td></tr>
      </tbody>
    </table>
  </div>

  <h3>High-rate flight instrumentation suite</h3>
  <div class="tw">
    <table class="spec">
      <caption>Telemetry and instrumentation channels</caption>
      <tbody>
        <tr><td><strong>Electrical</strong></td><td>Per-channel current draw, bus voltage drop, cell impedance</td></tr>
        <tr><td><strong>Thermal</strong></td><td>Motor windings, ESC heatsinks, battery cells, compute junction temps</td></tr>
        <tr><td><strong>Compute</strong></td><td>Microsecond bus latency, execution time histograms, cycle traces</td></tr>
        <tr><td><strong>Structural</strong></td><td>Multi-axis accelerations, fuselage harmonic frequency response</td></tr>
        <tr><td><strong>Logging fidelity</strong></td><td>Lossless microsecond black-box logger; every sensor tick preserved</td></tr>
        <tr><td><strong>Analysis turnaround</strong></td><td>Ingested and replayed against simulation predictions within hours</td></tr>
      </tbody>
    </table>
  </div>

  <figure>
    <img src="../assets/slides/apollyon-eng-5.jpg" alt="Flight at the edge, mismatch analysis, and the four systems it upgrades">
    <figcaption><b>Fig. 04</b> The iteration cadence: instrumented flight at the envelope edge feeds direct updates into physics, control, compute, and hardware.</figcaption>
  </figure>
</section>

<section id="architecture">
  <div class="sec-tag">05 · Direct-actuator policy</div>
...
<section id="generalisation">
  <div class="sec-tag">06 · Fleet generalisation</div>
  ...
  <figure>
    ...
    <figcaption><b>Fig. 05</b> The hardware data flywheel: dense instrumented flight data reinforces the physics core, accelerating development for subsequent airframe programs.</figcaption>
  </figure>
</section>

<section id="usedby">
  <div class="sec-tag">07 · Used by</div>
...
print("Single coherent Robust Control page written successfully.")
